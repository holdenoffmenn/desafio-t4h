#!/usr/bin/env python3
"""Treina e persiste o roteador semântico de intenções (sklearn Pipeline)."""
...
DATA_PATH = ROOT / "data" / "intents.jsonl"
OUT_PATH = ROOT / "models" / "intent_router.joblib"


def load_dataset(path: Path) -> tuple[list[str], list[str]]:
    """Carrega dataset rotulado de intenções.
...
def train(texts: list[str], labels: list[str]) -> Pipeline:
    ...
    Args:
        ...
        labels: Classes correspondentes.
    ...
    # Retreina no dataset completo para o artefato final
    pipeline.fit(texts, labels)
    return pipeline


def main() -> int:
    """Treina o roteador e salva ``models/intent_router.joblib``."""
